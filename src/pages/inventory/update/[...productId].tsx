...
export default function ProductList({ sellerData, brandData }: { sellerData: SellerData, brandData: any }) {
    ...
    useEffect(() => {
        if (productData?.data?.productImagesArray) {
            setObjectKeys(productData?.data?.productImagesArray);
        }

        if (productData?.data?.productMargin) {
            setProdMargin(productData?.data?.productMargin)
        }

        if (productData?.data?.productType) {
            setProductType(productData?.data?.productType)
        }

        if (productData?.data?.productCategory) {
            setProductCategory(productData?.data?.productCategory)
        }

        if (productData?.data?.productSubCategory) {
            setSubCategory(productData?.data?.productSubCategory)
        }

        if (productData?.data?.productVariations) {
            setVariationValues(productData?.data?.productVariations)
        }

        if (productData?.data?.variantOptions) {
            setVariantOptions(productData?.data?.variantOptions)
        }
    ...
    async function onSubmit(values: { productName: string, productCategory: string, productColor: string, productSize: string, productQuantity: string, productDescription: string, productSku: string, productSubCategory: any, productPrice: string, productCost: string, productMargin: string, productKeywords: string, productSizeValue: string, productType: string }) {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/inventory/products/${sellerData?.data?.id}/${productData?.data?.id}`, {
                method: 'PATCH',
        ...
        async function fetchData() {
            try {
                ...
                formik.setValues({
                    ...
                    productSizeValue: '', // Add this line
        ...
        <Layout>
            ...
            <div className="py-6 h-screen">
                <div className="mx-auto px-4 sm:px-6 md:px-8 ">
                    <Breadcrums parent={"Inventory"} childarr={["Update Product"]} />
                </div>
                        ...
                        <form onSubmit={formik.handleSubmit} >
                            <div className="bg-white shadow-[0_2px_8px_rgb(0,0,0,0.1)] rounded-lg p-7">
                                ...
                                <div className="mt-16 flex">
                                    <button type="submit" className="w-36 h-11 bg-[#F12D4D] flex items-center justify-center rounded-md text-white text-base font-semibold mr-5 cursor-pointer" value="Update">{loading ? <AiOutlineLoading3Quarters className='spinner' /> : `Update Product`}</button>
